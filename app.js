...
async function callApi(path) {
    try {
        ...
        // Deteksi data dalam result.data atau langsung result
...
async function renderDrama(path, label) {
    ...
    items.forEach(item => {
        // Ambil ID dan Nama secara fleksibel
...
async function openDetail(bid, bname, intro) {
    ...
    try {
        // PERBAIKAN: Menggunakan bookId (CamelCase)
        const episodes = await callApi(`/allepisode?bookId=${bid}`);
        epList.innerHTML = '';

        if (!episodes || episodes.length === 0) {
            epList.innerHTML = `<p class="text-yellow-500 text-sm italic text-center py-4">Episode belum tersedia untuk drama ini.</p>`;
        } else {
            episodes.forEach((ep, i) => {
                ...
                const epTitle = ep.episodeName || ep.title || `Episode ${i + 1}`;
                ...
                // DETEKSI URL VIDEO: Mencoba berbagai kemungkinan field name
                const playUrl = ep.videoUrl || ep.url || ep.play_url || ep.video_url;

                btn.onclick = () => {
                    if (playUrl) {
                        window.open(playUrl, '_blank');
                    } else {
                        alert("Maaf, link video belum tersedia di server.");
                    }
                };
...
function changeTab(type, el) {
    ...
    const config = {
        ...
        'foryou': { p: '/foryou', l: 'REKOMENDASI (FOR YOU)' },
